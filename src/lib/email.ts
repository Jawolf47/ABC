...
export async function sendProgressReminder(data: ProgressReminderData) {
  const apiKey = process.env.RESEND_API_KEY
  if (!apiKey) {
    console.log("[email] No RESEND_API_KEY set. Would have sent progress reminder to", data.email)
    return
  }
  ...
  const html = `
    ...
    <body style="font-family: -apple-system, sans-serif; background: #f5f5f5; padding: 40px 20px;">
      <table style="max-width: 560px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden;">
        <tr>
          <td style="padding: 32px 32px 0;">
            <h1 style="font-size: 24px; margin: 0; color: #d97706;">Log Your Progress</h1>
            <p style="color: #666; margin-top: 8px;">Hi ${data.name}, you recently attended a session at Alpha Bear Club.</p>
          </td>
        </tr>
        <tr>
          <td style="padding: 24px 32px;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 12px 0; border-bottom: 1px solid #eee; color: #666; font-size: 14px;">Session</td>
                <td style="padding: 12px 0; border-bottom: 1px solid #eee; text-align: right; font-size: 14px; font-weight: 600; color: #1a1a1a; text-transform: capitalize;">${data.type}</td>
              </tr>
              <tr>
                <td style="padding: 12px 0; border-bottom: 1px solid #eee; color: #666; font-size: 14px;">Date</td>
                <td style="padding: 12px 0; border-bottom: 1px solid #eee; text-align: right; font-size: 14px; font-weight: 600; color: #1a1a1a;">${data.date}</td>
              ...
              <tr>
                ...
                <td style="padding: 12px 0; text-align: right; font-size: 14px; font-weight: 600; color: #1a1a1a;">${data.timeSlot}</td>
              </tr>
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding: 0 32px 32px;">
            <p style="color: #666; font-size: 14px; margin: 0 0 16px;">
              Tracking your progress helps you see improvement over time and get personalized coaching feedback.
            </p>
            <a href="https://alpha-bear-club.vercel.app/account/progress" style="display: inline-block; background: #d97706; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">
              Log Your Progress
            </a>
          </td>
        </tr>
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Alpha Bear Club <bookings@alphabearclub.com>",
        to: [data.email],
        subject: "Log Your Progress - Alpha Bear Club",
        html,
      }),
    })
    ...
    if (!res.ok) {
      ...
      console.log("[email] Progress reminder sent to", data.email)
    }
  } catch (err) {
    console.error("[email] Failed to send progress reminder:", err)
  }
}
